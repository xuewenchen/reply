...
func sharding(id int64) string {
	return fmt.Sprintf("%02d", id%10)
}

func (d *Dao) AddReply(c context.Context, reply *model.Reply) (affected int64, err error) {
	result, err := d.db.Exec(c, fmt.Sprint(_addReplySQL, sharding(reply.SourceId)), reply.TypeId, reply.SourceId, reply.Comment, reply.ParentId, reply.Path)
	if err != nil {
		log.Error("AddReply(%+v) error(%v)", reply, err)
		return
	}
...
func (d *Dao) SelLimitReply(c context.Context, sourceId int64, typeId int8) (rs []*model.Reply, err error) {
	rows, err := d.db.Query(c, fmt.Sprint(_selLimitReplySQL, sharding(sourceId)), sourceId, typeId)
	if err != nil {
		log.Error("SelLimitReply(%d,%d) error(%v)", sourceId, typeId, err)
		return
	}
	for rows.Next() {
		r := &model.Reply{}
...
func (d *Dao) SelAllReply(c context.Context, sourceId int64, typeId int8) (rs []*model.Reply, err error) {
	rows, err := d.db.Query(c, fmt.Sprint(_selAllReplySQL, sharding(sourceId)), sourceId, typeId)
	if err != nil {
		log.Error("SelLimitReply(%d,%d) error(%v)", sourceId, typeId, err)
		return
	}
